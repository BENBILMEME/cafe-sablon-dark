  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'bg-[#0f1411]/92 backdrop-blur-xl border-b border-white/[0.04]'
          : 'bg-transparent'
      }`}
    >
      <nav
        className="mx-auto flex h-16 md:h-20 max-w-[1440px] items-center justify-between
                   px-6 md:px-12 lg:px-16"
        aria-label="Ana navigasyon"
      >
        {/* Logo */}
        <a href="/" className="flex items-center gap-2.5">
          <span className="font-serif text-xl md:text-2xl font-semibold text-[#e8e4db] tracking-tight">
            Brekkie
          </span>
          <span className="hidden sm:block w-px h-4 bg-[#D4A853]/30" aria-hidden="true" />
          <span className="hidden sm:block font-sans text-[10px] font-medium text-[#7d8c7f] tracking-[0.2em] uppercase">
            Breakfast Club
          </span>
        </a>
          ...
          {NAV_LINKS.map((link) => (
            <li key={link.href}>
              <a
                ...
                className="px-4 py-2 text-sm font-medium text-[#b0bab2] hover:text-[#e8e4db] cursor-pointer
                           transition-colors duration-200 rounded-lg hover:bg-white/[0.04]"
              >
                {link.label}
              </a>
            </li>
          ...
          <li className="ml-4">
            <motion.a
              ...
              className="px-5 py-2.5 bg-[#D4A853] text-[#0f1411] text-sm font-semibold
                         rounded-full hover:bg-[#e0c878] transition-colors duration-300
                         shadow-[0_4px_20px_rgba(212,168,83,0.2)]"
            >
              {t.nav.directions}
            </motion.a>
          </li>
        ...
        >
          <div className="w-5 h-3 relative flex flex-col justify-between">
            <span className={`block h-px bg-[#d4dcd5] transition-all duration-300 origin-center ${
              mobileOpen ? 'rotate-45 translate-y-[5px]' : ''
            }`} />
            <span className={`block h-px bg-[#d4dcd5] transition-all duration-200 ${
            ...
            <ul className="px-6 py-6 space-y-1 max-w-[1440px] mx-auto">
              {NAV_LINKS.map((link, i) => (
                <motion.li
                  ...
                  transition={{ delay: i * 0.06, ...snapTransition }}
                >
                  <a
                    href={link.href}
                    onClick={(e) => handleNavClick(e, link.href)}
                    className="block py-3 font-serif text-xl text-[#b0bab2] hover:text-[#e8e4db] cursor-pointer
              ...
              >
                <a
                  ...
                  className="inline-block mt-4 px-6 py-3 bg-[#D4A853] text-[#0f1411] font-semibold
                             rounded-full hover:bg-[#e0c878] transition-colors duration-300"
                >
                  Google Maps'te Aç
                </a>
